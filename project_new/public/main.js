...
function getPriceRange(price) {
  if (!price) return '700';
  const [min, max] = price.split(' - ').map(p => parseInt(p.replace(/\s/g, '')));
  if (max) {
    if (max <= 700) return '700';
    if (max <= 1700) return '700-1700';
    if (max <= 3000) return '1700-3000';
    return '3000';
  }
...
const chartConfig = {
  ...
  restaurants: {
    ...
    chart2: {
      ...
      getData: (data) => {
        // Filter out restaurants with 'Промо' rating
        const validData = data.filter(item => item.Rate !== 'Промо');
        const sortedData = [...validData].sort((a, b) => parseFloat(b.Rate) - parseFloat(a.Rate));
        return {
          labels: sortedData.map(p => p.Name),
          datasets: [{
            label: 'Рейтинг',
...
function applyFilters() {
  ...
  filteredData = mockData[activeCategory].filter(item => {
    if (currentFilters.minRate) {
      const rate = item.Rate === 'Промо' ? 0 : parseFloat(item.Rate);
      if (rate < parseFloat(currentFilters.minRate)) return false;
    }
...
async function refreshData() {
  try {
    ...
    mockData = {
      tourist_places: data.tourist_places,
    ...
    document.querySelector('.container').innerHTML = `
      ...
      <p>Пожалуйста, убедитесь, что сервер запущен и база данных доступна.</p>
    `;
  }
}
